...
const dbInit = {
	...
	async v1_6DB(c) {
		...
		const ADD_COLUMN_SQL_LIST = [
			...
			`ALTER TABLE setting ADD COLUMN notice_title TEXT NOT NULL DEFAULT '我的邮局';`,
	...
	async intDB(c) {
		...
		try {
			await c.env.db.prepare(`
			  INSERT INTO setting (
				register, receive, add_email, many_email, title, auto_refresh, register_verify, add_email_verify, timezone
			  )
			  SELECT 0, 0, 0, 0, '我的邮局', 0, 1, 1, 8
