...
const emailService = {
	...
	async send(c, params, userId) {
		...
		emailData.createTime = formatDetailDate(new Date());
		const recipient = [];
	...
	async emailAddAtt(c, list) {
		...
		const tzOffset = Number(settings.timezone ?? 8);
		if (emailIds.length > 0) {
			...
			list.forEach(emailRow => {
				...
				if (emailRow.createTime) {
					emailRow.createTime = dayjs(emailRow.createTime).add(tzOffset, 'hour').format('YYYY-MM-DD HH:mm:ss');
				}
			});
		}
	},
	...
	async batchDelete(c, params) {
		...
		if (sendEmail) {
			const emailArr = sendEmail.split(',');
			const orConditions = emailArr.map(mail => like(email.sendEmail,`${left ? '%' : ''}${mail.trim()}${right ? '%' : ''}`));
			conditions.push(or(...orConditions));
		}

		if (toEmail) {
			const emailArr = toEmail.split(',');
			const orConditions = emailArr.map(mail => like(email.toEmail,`${left ? '%' : ''}${mail.trim()}${right ? '%' : ''}`));
			conditions.push(or(...orConditions));
		}
		...
		const emailIdsRow = await orm(c).select({emailId: email.emailId, sendEmail: email.sendEmail, toEmail: email.toEmail}).from(email).where(conditions.length > 1 ? and(...conditions) : conditions[0]).all();

		const emailIds = emailIdsRow.map(row => row.emailId);

		let resultInfo = { missingSend: [], missingTo: [] };
		if (sendEmail) {
			const arr = sendEmail.split(',').map(e => e.trim());
			resultInfo.missingSend = arr.filter(e => !emailIdsRow.some(r => r.sendEmail && r.sendEmail.includes(e)));
		}
		if (toEmail) {
			const arr = toEmail.split(',').map(e => e.trim());
			resultInfo.missingTo = arr.filter(e => !emailIdsRow.some(r => r.toEmail && r.toEmail.includes(e)));
		}
